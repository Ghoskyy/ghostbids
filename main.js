...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["ghoskyXbids", "Chrome", "3.0"];
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "remove" && !mem.includes(client.user.jid)) {
        ...
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm");
        memeg = mdata.participants.length;
        out = `Yah kak ${anu_user} keluarr\n\nSayonara kakaaa kami akan merindukan mu`;
        buff = await getBuffer(
        ...
        buttons = [
          { buttonId: `y`, buttonText: { displayText: "Byeee👋" }, type: 1 },
        ];
        imageMsg = (
          await client.prepareMessageMedia(buff, "imageMessage", {
            thumbnail: buff,
